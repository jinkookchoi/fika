...
- **고정 휴식 시간대** — 매일 정해진 시간(예: 점심 11:30~13:00)엔 작업 사이클을 멈추고 쉼. 시작 5분 전 예고 + 시작/끝 시각과 이름 설정. 화면은 덮지 않고 남은 시간만 표시
- **하루 마무리 알림** — 정한 시각 30분·15분 전과 도래 시 알려줌(강제 종료 없음). 도래 시 오늘 집중 요약 + "오늘은 그만"(일시정지) 버튼
...
휴식 알림 앱은 이미 좋은 게 많습니다. Fika는 그 사이에서 **가볍고 귀여운 쪽**을 택했습니다.

| | TomatoBar | Stretchly | **Fika** |
|---|:---:|:---:|:---:|
| 형태 | 네이티브, 가벼움 | Electron, 무거움 | **네이티브, 가벼움** |
| 휴식 강제 / 자리 비움 | ✕ | ○ | ○ |
| sleep/wake 보정, 복귀 대기 | ✕ | ✕ | ○ |
| 고정 시간대 휴식(점심 등) | ✕ | ✕ | ○ |
| 캐릭터 감성 | — | — | **커피 마스코트** |
| 기능 가짓수 | 적음(타이머만) | 매우 많음 | 적당 |

- **TomatoBar**는 순수 뽀모도로 타이머입니다. Fika는 거기에 휴식 강제, 자리 비움, 동작 알림까지 더했습니다.
- **Stretchly**는 기능이 가장 풍부하지만 Electron이라 무겁고 투박합니다. Fika는 가벼운 네이티브에 **커피 마스코트 감성**과 **sleep/wake 보정 같은 섬세한 디테일**로 차별화합니다.

> 한 줄로: *Stretchly의 휴식 케어 + TomatoBar의 가벼움 + 둘에 없는 귀여움.*
...
| **시간** | 작업 / 짧은 휴식 / 긴 휴식(시간, 주기) / 휴식 예고(초) / 연기 길이 / 고정 휴식 시간대 |
| **알림** | 메뉴바 아이콘 테마, 시간 표시 토글, 아이콘 미리보기, 남은 시간 알림(켬/끔, 주기), 하루 마무리 알림(마칠 시각), 휴식 알림 방식, 사운드 |
